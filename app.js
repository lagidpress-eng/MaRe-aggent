...
function updateProjectHeader(){
  ...
  if(sub) sub.textContent = `${PROJECT} · V10`;
}
...
function completionRequirements(record, pole){
  ...
  const hoaMissingDescription =
    hoaIsChanged && !String(record.heightChangeDescription || "").trim();
...
async function syncSheetToCloud(){
  ...
  await googleFetch(
    `https://sheets.googleapis.com/v4/spreadsheets/${spreadsheetId}/values/Field%20Log!A1:N100?valueInputOption=RAW`,
    {
      method:"PUT",
      headers:{"Content-Type":"application/json"},
      body:JSON.stringify({
        range:"Field Log!A1:N100",
        majorDimension:"ROWS",
        values:fieldValues
      })
    }
...
async function syncPhotosToCloud(){
  ...
  const unsynced = photos.filter(p => p.project === PROJECT && !p.driveFileId);
...
async function syncAllToCloud(){
  ...
  try{
    ...
    setCloudStatus(`☁️ Synced · ${signedInEmail || "Google"} · ${userRole}`, "connected");
  }finally{
    syncBusy = false;
  }
}
...
function validateCompletionDialog(){
  ...
  if(req.hoaMissingDescription){
    return {ok:false,message:"Height Change Description is required because ACTUAL HOA differs from PROJECT HOA."};
  }
...
function bindProjectEvents(){
  ...
  el("saveProjectSettingsBtn").addEventListener("click", () => {
    if(!requireAdmin()) return;
    if(!activeProject) return;
