...
function openPole(n){
 ...
 $("#saveMsg").textContent="";renderGalleries();$("#poleDialog").showModal();syncQuickStatusV5();
}
...
// ===== V5 Field card workflow =====
async function updatePhotoCounts(){
  if(!currentPole) return;
  const before = await dbGetByPoleType(currentPole.projectPole,"BEFORE");
  const after = await dbGetByPoleType(currentPole.projectPole,"AFTER");
  const b = document.getElementById("beforeCount");
  const a = document.getElementById("afterCount");
  if(b){
    b.textContent = `${before.length}/3`;
    b.classList.toggle("photo-ok", before.length >= 3);
  }
  if(a){
    a.textContent = `${after.length}/3`;
    a.classList.toggle("photo-ok", after.length >= 3);
  }
}
...
(function(){
  ...
  if(startBtn) startBtn.onclick=()=>{
    ...
    if(el) el.value="In progress";
    startBtn.classList.add("active");
    const done=document.getElementById("quickCompleteBtn");
    if(done) done.classList.remove("active");
  };

  const completeBtn=document.getElementById("quickCompleteBtn");
  if(completeBtn) completeBtn.onclick=()=>{
    const el=document.getElementById("status");
    if(el) el.value="Completed";
    completeBtn.classList.add("active");
    const start=document.getElementById("quickStartBtn");
    if(start) start.classList.remove("active");
  };

  const saveNext=document.getElementById("saveNextBtn");
  if(saveNext) saveNext.onclick=async()=>{
    const ok=await saveCurrentPole(false);
    if(ok){
      document.getElementById("poleDialog").close();
      window.open(MY_MAPS_URL,"_blank");
    }
  };
})();

const _renderGalleriesV5 = renderGalleries;
renderGalleries = async function(){
  await _renderGalleriesV5();
  await updatePhotoCounts();
};

function syncQuickStatusV5(){
  const s=document.getElementById("status")?.value;
  document.getElementById("quickStartBtn")?.classList.toggle("active",s==="In progress");
  document.getElementById("quickCompleteBtn")?.classList.toggle("active",s==="Completed");
}
document.getElementById("status")?.addEventListener("change",syncQuickStatusV5);
