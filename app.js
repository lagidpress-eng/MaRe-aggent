...
async function syncSheetToCloud(){
  ...
  await googleFetch(
    `https://sheets.googleapis.com/v4/spreadsheets/${spreadsheetId}/values/Field%20Log!A1:J100?valueInputOption=RAW`,
    {
      method:"PUT",
      headers:{"Content-Type":"application/json"},
      body:JSON.stringify({
        range:"Field Log!A1:J100",
        majorDimension:"ROWS",
        values:fieldValues
      })
    }
...
function bindEvents(){
  ...
  el("saveNextBtn").addEventListener("click", async () => {
    if(await saveAndMaybeSync()){
      el("poleDialog").close();
      window.location.assign(MY_MAPS_URL);
    }
  });
