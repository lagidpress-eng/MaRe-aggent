...
const GOOGLE_SCOPES = "openid email https://www.googleapis.com/auth/userinfo.email https://www.googleapis.com/auth/drive https://www.googleapis.com/auth/spreadsheets";
...
async function ensureDriveFileInFolder(fileId, folderId){
  let file = await getDriveFileIfExists(fileId);
  if(!file) throw new Error("Uploaded Drive file could not be found.");

  let parents = Array.isArray(file.parents) ? file.parents : [];
  if(!(parents.length === 1 && parents[0] === folderId)){
    await moveDriveFileToExactFolder(fileId, folderId);
    file = await getDriveFileIfExists(fileId);
    parents = Array.isArray(file?.parents) ? file.parents : [];
  }

  if(!(parents.length === 1 && parents[0] === folderId)){
    throw new Error(`Photo ${fileId} is still not inside the expected pole folder.`);
  }
  return true;
}
...
async function syncPhotosToCloud(){
  ...
  for(let i=0; i<photos.length; i++){
    ...
    if(item.driveFileId){
      ...
      if(existing){
        await ensureDriveFileInFolder(item.driveFileId, folderId);
    ...
    await ensureDriveFileInFolder(uploaded.id, folderId);
...
async function syncAllToCloud(){
  ...
  try{
    ...
    await syncSheetToCloud();
    await syncPhotosToCloud();
    setCloudStatus(`☁️ Synced · photos verified in pole folders · ${signedInEmail || "Google"} · ${userRole}`, "connected");
  }finally{
    syncBusy = false;
  }
}
